...
define(['underscore','utils/preloader','app/task/build_url'],function(_,preload,build_url){
	...
	var loadMedia = function(media){
		...
		if (!_.isUndefined(media.image)) preload.add(build_url(media.image, 'image'),'image');
		if (!_.isUndefined(media.template)) preload.add(build_url(media.template,'template'),'template');		
	}
	...
	var loadScript = function(script){
		// load media sets
		_.each(script.mediaSets || [], function(set,name){
