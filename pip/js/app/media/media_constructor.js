define(['jquery', './media_view','../inflator','utils/html','app/task/build_url'], function($, media,inflate,html,build_url) {
	...
	return function(options,model){
		...
		// if needed, build url
		if (definitions.template) definitions.template = build_url(definitions.template,'template');
		if (definitions.image) definitions.image = build_url(definitions.image,'image');
