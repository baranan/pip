define(['underscore','./current_trial'],function(_,current_trial){
	...
	return function(propositions, inputData){		
		...
		_.each(propositions,function(proposition){			
			...
			switch (proposition.type){
				case 'begin':
					if (inputData.type !== 'begin') evaluation = false;
					break;
									
				case 'inputEquals' :
					if (inputData.handle !== proposition.value) evaluation = false;
					break;
					
				case 'trialEquals':
					if (inputData.handle !== trial.data[proposition.value]) evaluation = false;
					break;
					
				case 'stimEquals':
					// create search object
					var searchObj = {};
					if (proposition.handle) searchObj['handle'] = proposition.handle; 
					...
					if (result.length == 0) evaluation = false;														
					break;
					
				case 'function' :
					if (!proposition.value.apply(trial,[trial,inputData])) evaluation = false;									
			}
